...
- Prefix detection (e.g., "runtime-scaffold", "ouroboros:")
...
class MagicKeywordDetector:
    ...
    1. Magic prefixes (e.g., "runtime-scaffold run", "/ouroboros:interview")
    ...
    def _iter_exact_prefix_variants(self) -> list[tuple[str, str]]:
        ...
        for skill_name, metadata in self._registry.get_all_metadata().items():
            ...
            prefixes.append(f"runtime-scaffold {skill_name}")
            if skill_name == "welcome":
                prefixes.extend(("runtime-scaffold", "/ouroboros", "ouroboros"))
...
def is_magic_command(
    ...
    if input_lower in ("runtime-scaffold", "/ouroboros", "ouroboros"):
        return True

    exact_patterns = (
        r"^runtime-scaffold:[a-z0-9_-]+(?:\s+.*)?$",
        r"^runtime-scaffold\s+[a-z0-9_-]+(?:\s+.*)?$",
