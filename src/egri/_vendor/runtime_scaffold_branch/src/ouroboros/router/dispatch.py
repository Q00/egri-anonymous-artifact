"""Shared deterministic router for ``runtime-scaffold`` skill dispatch.
...
def extract_first_argument(remainder: str | None) -> str | None:
    ...
    natural-language usage like ``runtime-scaffold interview add dark mode to settings``
    yields the full phrase rather than just ``add``. Quoted forms such as
    ``runtime-scaffold interview "add dark mode"`` produce the same unquoted result. If
...
def _validate_parsed_command(parsed: ParsedOooCommand) -> str | None:
    ...
    valid_prefixes = (f"runtime-scaffold {parsed.skill_name}", f"/ouroboros:{parsed.skill_name}")
...
class SkillDispatchRouter:
    """Stateless resolver for deterministic ``runtime-scaffold`` skill dispatch.
