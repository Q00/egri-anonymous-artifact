"""Live portability matrix scaffolding for EGRI.
...
from egri.memory import build_memory_observations_from_cell
from egri.memory import LocalJsonMemoryBackend
from egri.memory import MemoryBackend
from egri.memory import MemoryMode
from egri.memory import MemoryPrior
from egri.memory import NoopMemoryBackend
from egri.memory import READ_MODES
from egri.memory import WRITE_MODES
from egri.traceguard import build_manifest_from_fixture
from egri.traceguard import CLAIM_FACT_KEYS
from egri.traceguard import CLAIM_FACT_LIST_KEYS
from egri.traceguard import CHUNK_KEYS
from egri.traceguard import extract_parent_claims
from egri.traceguard import normalize_allowed_evidence_manifest
from egri.traceguard import TraceGuardEvidence
from egri.traceguard import TraceGuardRejection
...
PRIMARY_CONTRACT_VARIANT = "egri_traceguard"
DEFAULT_OUTPUT_PREFIX = "live-portability-matrix"
ARTIFACT_ROOT = Path(__file__).resolve().parents[2]
UPSTREAM_OUROBOROS_ROOT = Path("<USER_HOME>/Project/runtime-scaffold")
...
RUNTIME_FAMILIES = (
    RuntimeFamily(
        ...
        command_name="hermes",
        model_env_var="EGRI_HERMES_GLM_MODEL",
    ...
    RuntimeFamily(
        ...
        command_name="claude",
        model_env_var="EGRI_CLAUDE_MODEL",
    ...
    RuntimeFamily(
        ...
        command_name="codex",
        model_env_var="EGRI_CODEX_MODEL",
...
def generate_primary_fixtures(*, count: int = 8) -> list[dict[str, Any]]:
    ...
    specs = [
        ...
        _fixture_spec(
            ...
            [
                "The omitted note says EGRI beats all baselines.",
...
def build_dry_plan(
    *,
    ...
    return {
        "schema_version": "egri.live_portability_plan.v1",
        "run_mode": "dry_plan",
        "generated_at": _now(),
...
def run_contracts_only(
    *,
    ...
    return {
        **plan,
        "schema_version": "egri.live_portability_contracts_only.v1",
...
async def run_live_smoke(
    *,
    ...
    return {
        "schema_version": "egri.live_portability_smoke.v1",
        "run_mode": "live_smoke",
        "generated_at": _now(),
...
async def run_live_primary(
    *,
    ...
    """Run the primary EGRI+TG matrix with live runtime calls."""
    memory_context = memory_context or MemoryContext()
    fixtures = generate_primary_fixtures(count=fixture_count)
...
def _build_live_primary_result(
    *,
    ...
    return {
        "schema_version": "egri.live_portability_primary.v1",
...
def markdown_report(result: dict[str, Any]) -> str:
    title = {
        "dry_plan": "EGRI Live Portability Matrix Plan",
        "contracts_only": "EGRI Live Portability Contracts-Only Check",
        "live_smoke": "EGRI Live Portability Smoke Run",
        "live_primary": "EGRI Live Portability Primary Run",
    }.get(result["run_mode"], "EGRI Live Portability Matrix")
    lines = [
        f"# {title}",
        "",
...
def _build_fixture(spec: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    fixture = {
        "schema_version": "egri.live_portability_fixture.v1",
...
def _interpretation(result: dict[str, Any]) -> str:
    ...
    if result["run_mode"] == "live_smoke":
        ...
        return (
            f"Live smoke mode executes one EGRI+TG fixture across "
    ...
    if result["run_mode"] == "live_primary":
        ...
        return (
            f"Live primary mode executes the EGRI+TG contract for "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--memory-store",
        type=Path,
        default=Path(".egri-memory.jsonl"),
