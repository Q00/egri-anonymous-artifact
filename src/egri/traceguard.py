...
def validate_parent_synthesis(
    *,
    ...
    claims = extract_parent_claims(parent_synthesis)
    if not claims:
        rejected.append(
            TraceGuardRejection(
                reason="no_claim_bearing_surface",
                claim=TraceGuardClaim(
                    fact_id=None,
                    chunk_id=None,
                    surface="parent_synthesis",
                    text=(
                        "Parent synthesis did not expose any structured retained/"
                        "observed fact or evidence-reference claims."
                    ),
                ),
                detail=(
                    "TraceGuard fails closed when parent synthesis contains no "
                    "claim-bearing surface; prompt/schema wrappers are not accepted "
                    "as validated parent answers."
                ),
            )
        )
    for claim in claims:
