...
Cleanly reverses everything `ouroboros setup` did:
  ...
  2. CLAUDE.md integration block (<!-- runtime-scaffold:START --> … <!-- runtime-scaffold:END -->)
...
def _remove_claude_md_block(project_dir: Path, dry_run: bool) -> bool:
    """Remove <!-- runtime-scaffold:START --> … <!-- runtime-scaffold:END --> block from CLAUDE.md."""
    ...
    try:
        ...
        return False
    if "<!-- runtime-scaffold:START -->" not in content:
        return False

    if dry_run:
        print_info(f"[dry-run] Would remove runtime-scaffold block from {claude_md}")
        return True

    cleaned = re.sub(
        r"<!-- runtime-scaffold:START -->.*?<!-- runtime-scaffold:END -->\n?",
        "",
        content,
        flags=re.DOTALL,
    )
...
def uninstall(
    ...
    try:
        if claude_md.exists() and "<!-- runtime-scaffold:START -->" in claude_md.read_text():
