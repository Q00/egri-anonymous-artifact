#!/usr/bin/env python3
"""Exercise the in-process TraceGuard gate for project-local ``runtime-scaffold``.

This experiment does not rerun Hermes. It validates the exact persisted paper
``runtime-scaffold`` parent output through the deterministic adapter used by the
project-local ``uv run runtime-scaffold`` / ``uv run runtime-scaffold`` wrappers, then
...
from egri.runtime_scaffold_traceguard import validate_runtime_scaffold_result
from egri.runtime_scaffold_traceguard import install_runtime_scaffold_cli_gate


DEFAULT_INPUT = Path("experiments/paper-key-sections-runtime-scaffold-demo.json")
DEFAULT_JSON_OUTPUT = Path("experiments/runtime-scaffold-inprocess-traceguard-gate.json")
DEFAULT_MD_OUTPUT = Path("experiments/runtime-scaffold-inprocess-traceguard-gate.md")
...
def _run(input_path: Path) -> dict[str, Any]:
    ...
    patch_installed = install_runtime_scaffold_cli_gate()
    ...
    for case_id, parent_output in (
        ...
        gate = validate_runtime_scaffold_result(_demo_result(demo, parent_output))
        cases.append(
            {
                "case_id": case_id,
                "status": gate.status,
    ...
    return {
        "schema_version": "egri.runtime_scaffold_inprocess_traceguard_gate.v1",
        ...
        "cases": cases,
        "conclusion": {
            "wrapper_patch_available_in_current_dependency": patch_installed,
...
def _markdown(result: dict[str, Any]) -> str:
    lines = [
        "# runtime-scaffold In-Process TraceGuard Gate",
        "",
        "This artifact exercises the deterministic adapter used by the project-local",
        "`uv run runtime-scaffold` and `uv run runtime-scaffold` entrypoints. It uses the",
    ...
    if not result["patch_installed"]:
        lines.append(
            "- Patch note: the installed anonymous-review dependency did not expose "
            "the upstream CLI hook, so this run validates the persisted parent "
            "through the same deterministic gate without mutating the dependency."
        )
    lines.extend(
        [
            "",
