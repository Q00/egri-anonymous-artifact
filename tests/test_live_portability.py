...
import egri.live_portability as live_portability
from egri.live_portability import CONTRACT_VARIANTS
from egri.live_portability import RUNTIME_FAMILIES
from egri.live_portability import build_dry_plan
from egri.live_portability import generate_primary_fixtures
from egri.live_portability import run_contracts_only
from egri.live_portability import run_live_primary
from egri.live_portability import validate_fixture_contracts
...
@pytest.mark.asyncio
async def test_live_cell_rejects_prompt_wrapper_parent_synthesis_without_claims(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture = generate_primary_fixtures(count=8)[0]
    prompt_wrapper_parent = {
        "task": "synthesize_parent_answer_from_child_evidence",
        "child_records": ["prompt payload, not a model answer"],
        "required_schema": fixture["safe_parent_synthesis"],
        "rules": ["emit retained_facts"],
    }

    artifact = await _run_live_cell_without_provider_calls(
        monkeypatch=monkeypatch,
        fixture=fixture,
        parent_synthesis=prompt_wrapper_parent,
    )

    traceguard_validation = cast(dict[str, Any], artifact["traceguard_validation"])

    assert artifact["status"] == "primary_contract_failure"
    assert artifact["failure_classification"] == "primary_contract_failure"
    assert artifact["mandatory_contract_pass"] is False
    assert traceguard_validation["accepted"] is False
    assert traceguard_validation["accepted_claims"] == []
    assert traceguard_validation["rejected_claims"][0]["reason"] == (
        "no_claim_bearing_surface"
    )
...
def test_brownfield_seed_is_parseable_and_secret_safe() -> None:
    ...
    if not seed_path.exists():
        pytest.skip("optional brownfield live-portability seed is not included in anonymous bundle")
    ...
    assert "<USER_HOME>/Project/runtime-scaffold-adapter" in serialized
    assert "<USER_HOME>/Project/runtime-scaffold" in serialized
