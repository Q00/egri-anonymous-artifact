#!/usr/bin/env python3
"""Validate the paper runtime-scaffold demo with TraceGuard.

This script does not rerun Hermes. It takes the persisted output of the actual
dependency `runtime-scaffold` paper run, normalizes the parent synthesis claims
into TraceGuard's fact/evidence-handle contract, and validates two cases:

1. the supported parent synthesis from the runtime-scaffold run;
2. the same parent synthesis with an injected unsupported memory-answer claim.

The goal is to prove the post-run end-to-end gate for this exact runtime-scaffold paper
...
DEFAULT_JSON_OUTPUT = Path("experiments/paper-runtime-scaffold-traceguard-gate.json")
DEFAULT_MD_OUTPUT = Path("experiments/paper-runtime-scaffold-traceguard-gate.md")
...
def _build_manifest_and_parent(
    ...
    parent_synthesis = {
        "mode": "paper_runtime_scaffold_parent_synthesis_traceguard_normalized",
        "verdict": "completed",
        "result": {
            "summary": (
                "Normalized parent claims from the actual paper runtime-scaffold run."
            ),
...
def _unsafe_parent(parent_synthesis: dict[str, Any]) -> dict[str, Any]:
    ...
    unsafe["result"]["retained_facts"].append(
        {
            "fact_id": "MEMORY-ANSWER-RUNTIME-SCAFFOLD-001",
            "text": (
...
def _run(input_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "egri.paper_runtime_scaffold_traceguard_gate.v1",
        ...
        "conclusion": {
            "exact_runtime_scaffold_run_traceguard_safe_accepts": safe_result.accepted,
            "exact_runtime_scaffold_run_traceguard_rejects_memory_answer": (
                not unsafe_result.accepted
            ),
            "automatic_gate_scope": (
                "This script is the automatic post-run TraceGuard gate for the "
                "persisted runtime-scaffold paper run. The upstream dependency "
                "`runtime-scaffold` command did not call this validator internally "
                "when this run was recorded; the project-local `runtime-scaffold`/`ouroboros` "
...
def _markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "| --- | ---: | ---: | --- |",
        "| exact runtime-scaffold parent | {accepted} | {rate:.4f} | {reasons} |".format(
