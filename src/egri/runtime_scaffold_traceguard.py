"""TraceGuard integration for the dependency ``runtime-scaffold`` CLI.
...
class RuntimeScaffoldTraceGuardGate:
    """Result of the in-process TraceGuard gate for one ``runtime-scaffold`` run."""
...
def validate_runtime_scaffold_result(result: Any) -> RuntimeScaffoldTraceGuardGate:
    """Validate a completed dependency ``runtime-scaffold`` result in-process."""
    ...
    return RuntimeScaffoldTraceGuardGate(
...
def install_runtime_scaffold_cli_gate() -> bool:
    """Patch the installed runtime-scaffold CLI helper for this process.
    ...
    marker = "_egri_traceguard_original_run_with_default_trace_store"
    ...
    async def run_with_traceguard(*args: Any, **kwargs: Any) -> Any:
        ...
        gate = validate_runtime_scaffold_result(result)
...
def _not_applicable(reason: str) -> RuntimeScaffoldTraceGuardGate:
    return RuntimeScaffoldTraceGuardGate(
...
def _rejected(reason: str) -> RuntimeScaffoldTraceGuardGate:
    return RuntimeScaffoldTraceGuardGate(
        status="rejected",
        validation=None,
        manifest=(),
...
def _normalize_parent_synthesis(
    ...
    *,
    ...
    return (
        {
            "mode": "runtime_scaffold_parent_synthesis_traceguard_normalized",
...
def _parent_summary(parent_output: Mapping[str, Any]) -> str:
    ...
    if isinstance(result, Mapping):
        for key in ("parent_summary", "summary"):
            ...
            if isinstance(item, str) and item.strip():
                return item
    return "Normalized dependency runtime-scaffold parent synthesis."
